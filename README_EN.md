...
| Local Python SDK | Implemented | The default mock backend queries securities, calendars, prices, trading constraints, PIT fundamentals, index/industry membership, universes, factors, and health data offline. Public `get_factor` and `get_adjustment_factor` support only `query_mode="latest"`; because their current signatures cannot express a knowledge cutoff or pinned data version, `asof` and `vintage` fail closed. |
...
| ClickHouse vintage migration selector | Locally integration-tested | Local Docker with ClickHouse 24.8.14.39 covered fresh old-key full schemas and four source rows in one old-key part through market/factor create-copy-EXCHANGE, old-key backup, and post-migration OPTIMIZE FINAL checks. This evidence is limited to the migration selector and latest factor revision selection, not production operation; CI does not run database integration. |
| PostgreSQL query selectors | Partially integration-tested locally | A disposable Postgres 16 database was built from `0001`, `0006`, `0058`–`0060`, and the seed. Real psycopg calls exercised PostgreSQL array binding, `DISTINCT ON`, stable-`security_id` ticker renames, historical security identities and industry labels, PIT fundamentals/memberships, successful-batch admission, knowledge cutoffs, suspension-only trading-constraint revisions, empty universe snapshots, and same-day reruns. The market-data boundary remained a deterministic fake; query plans, cross-store transactions, failure recovery, performance, and sustained operation remain unverified, and CI does not run database integration. |
...
The API boundary is intentionally narrow: the price API has enough parameters to express `latest`, `asof`, and `vintage`, while the public adjustment-factor and factor-value APIs currently return only a deterministic latest revision. A factor request's `start_date`/`end_date` filter economic dates; it is not a historical knowledge cutoff. Reproducible research should consume a verified snapshot instead of treating that date filter as PIT.

The SQL master-data producer also fails closed: a symbol/name-only placeholder may establish a current market-data mapping, but it writes no PIT identity, name, or status history; a ticker rename must provide a stable `security_id` and the corresponding effective dates. Industry assignments and labels are selected from historical successful-batch revisions under the knowledge cutoff, and a rule-based universe cannot reuse an existing non-`rule_based` code. Trading constraints use the union of limit-price observations and suspension episodes as their date spine; a filtered universe excludes members whose constraint evidence is missing.
...
The database containers, migrations, and SQL backend are outside the offline green path. The ClickHouse migration selector was locally exercised in Docker on ClickHouse 24.8.14.39 using fresh old-key full schemas and four source rows in one old-key part, including market/factor create-copy-EXCHANGE, old-key backup, and OPTIMIZE FINAL. This is not end-to-end production-backend evidence. On PostgreSQL, a disposable Postgres 16 database exercised real array binding, `DISTINCT ON`, PIT membership/fundamental, historical-identity, batch/cutoff/revision, and universe-snapshot selection while the ClickHouse market-data boundary remained fake. query plans, cross-store transactions, failure recovery, performance, and sustained operation still need real integration testing, and CI does not run database integration. In particular, a migration that fixes a ClickHouse sorting key protects future merges only; vintages already collapsed under the old key cannot be recovered by that migration and must be rebuilt from retained source data or an earlier verified snapshot.
